...
add_action('wp_head', 'theme_share_meta');

/**
 * Print open graph and twitter card meta tags
 */
function theme_share_meta() {
	...
	if ( is_home() ) {
		...
		$meta['title'] = trim(theme_get_document_title());
	...
	$meta['image'] = false;
	$default_image = get_bloginfo('template_directory') . '/images/news-placeholder.png'; // Set to something useful
	...
	if ( has_post_thumbnail() && !is_archive() ) {
		...
		$first_image = get_field( 'gallery-gallery' );
		if ( $first_image ) {
			$meta['image'] = $first_image[0]['sizes']['large'];
		} else {
			$meta['image'] = $default_image;
		}
	} elseif ( get_post_type() == 'video' ) {
		$video = get_field( 'video-video' );
		if ( $video ) {
			$meta['image'] = sh_get_video_thumbnail( $video, 'maxresdefault' );
		} else {
			$meta['image'] = $default_image;
		}
